...
NormalViz::NormalViz (ros::NodeHandle &anode) : nh_(anode) {
  nh_.param("input_cloud_topic", input_cloud_topic_, std::string("/DEBUG/PointCloud"));
  ...
  for (unsigned int i = 0; i < size; ++i)
    {
      ...
      //axis-angle rotation
	ROS_INFO(": %f %f %f", pcl_cloud.points[i].normal[0], pcl_cloud.points[i].normal[1], pcl_cloud.points[i].normal[2]);
      ...
      normals_marker_array_msg_.markers[i].header.stamp = ros::Time::now();//pcl_cloud.header.stamp;
      ...
      normals_marker_array_msg_.markers[i].lifetime = ros::Duration(1);
      normals_marker_array_msg_.markers[i].type = visualization_msgs::Marker::ARROW;
      normals_marker_array_msg_.markers[i].scale.x = 0.02;
      normals_marker_array_msg_.markers[i].scale.y = 0.02;
      normals_marker_array_msg_.markers[i].scale.z = 0.02;
